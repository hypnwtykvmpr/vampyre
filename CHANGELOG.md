...
- Security: semantic extraction now blocks compound credential assignments such as `AWS_SECRET_ACCESS_KEY`, `SECRET_KEY`, and punctuation-rich password values before document, text, or YAML content can reach an LLM provider. The detector still permits placeholders, environment references, and security-related source names. The optional PDF dependency is floored at `pypdf>=6.15.0`, whose upstream security release bounds whitespace-token, CID-font-width, and ToUnicode-token parsing work ([#3947](https://github.com/py-pdf/pypdf/pull/3947), [#3946](https://github.com/py-pdf/pypdf/pull/3946), [#3944](https://github.com/py-pdf/pypdf/pull/3944)); Graphify parses untrusted PDFs on its document path.
- Breaking/Fix: multipart node IDs now retain part boundaries with a full deterministic digest, preventing distinct paths such as `("mod", "a_b")` and `("mod_a", "b")` from collapsing onto one node. The graph profile records AST and semantic node-ID schemas separately from the package version, and their caches are namespaced by the corresponding identity inputs so an unchanged `0.9.5` version cannot reuse legacy IDs. An ordinary full AST-only `update` migrates unmarked AST state without an LLM by remapping carried semantic, legacy, keyed-edge, and hyperedge references only when each old AST node has one unique source-identity match; ambiguity refuses the entire write. Partial watch/hook updates refuse legacy AST state until that migration runs. Preserved semantic state remains explicitly marked `legacy-preserved`; `graphify update --remap` performs the full semantic re-extraction required before incremental semantic extraction. Semantic cache entries that predate backend/model/schema provenance are deliberately not reused and are pruned after a safe re-extraction. The confined `claude-cli` backend now refuses image extraction with actionable inline-vision guidance instead of granting the coding agent filesystem tools; text semantic extraction remains available.
- Fix: assistant-skill freshness is now destination-local and content-verified, so installing one platform cannot certify stale files for another. The installed CLI now exposes the existing MCP server as `graphify serve` while retaining `graphify-mcp` for external client configuration. `graphify extract --force` and `GRAPHIFY_FORCE=1` now have one narrow meaning: accept a verified non-empty shrink without clearing/bypassing caches or implying `--full`; empty-over-populated writes remain forbidden.
- Fix: make the supported development and CI surface genuinely cross-platform. The blocking test matrix now runs the full suite on Windows, macOS, and Linux with Python 3.10 and 3.13, using grouped parallel execution so packaging tests do not race over shared build state. The suite fails on skips, deselections, expected/unexpected xfail outcomes, and warnings; test homes and provider environments are isolated; symlink coverage uses Windows junctions when link privileges are unavailable; serialized paths recognize both Windows and POSIX absolute forms on every host; Git ancestor discovery honors `GIT_CEILING_DIRECTORIES`; and exports escape Windows device filenames. Text files are normalized to LF and production text I/O now declares UTF-8 explicitly.
- Fix: graph-state publication is now crash- and race-safe across extract, update/watch, cluster-only, merge, and global-graph operations. Canonical JSON, manifests, markers, reports, labels, and protected backups use atomic writes with rollback on a late failure; concurrent rebuild queues and semantic-update signals retain unacknowledged generations; source bytes are revalidated before manifest advancement so a file changed during extraction cannot be marked current. Installed assistant skills use the same strict profile/marker contract and preserve keyed multigraph edges instead of running a separate lossy merge. Read-only loading still repairs losslessly interpretable legacy forms (including `edges`/`links`, omitted legacy class fields, and missing multigraph keys), but malformed records that would have to be silently discarded now fail loudly instead of producing a partial graph.
- Breaking/Fix: `graphify update` and `graphify watch` now require an existing `graph.json` with an explicit `simple`, `digraph`, or `multidigraph` profile and a valid scan-root marker. Missing, malformed, or ambiguous state fails before mutation instead of silently creating a rival unmarked/simple graph. Re-run `graphify extract <path>` with `--simple`, `--directed`, or `--multigraph` to migrate older unprofiled graphs. Both commands now accept `--out` for graph state stored outside the source tree; locks, queues, caches, manifests, reports, hooks, and exports follow that canonical output. `update --remap` now performs a genuine full extraction while preserving the exact graph class.
...
- Fix: clustered `extract`, `update`/watch, and `cluster-only` now keep `.graphify_analysis.json` transactionally aligned with `graph.json` (#1617 / #1610, thanks @sanmaxdev). The sidecar binds its community membership and analysis to a canonical graph-content fingerprint, so a stale or copied sidecar is repaired instead of surviving a no-change fast path. A changed `--no-cluster` update removes stale analysis/report/visual outputs and old per-node community claims; a true no-op preserves them byte-for-byte.
...
## Earlier 0.9.5 development history (through 2026-07-02)
...
- Docs: A file-level node-summary RFC was evaluated and later retired without becoming a supported command or graph-state contract.
...
- Fix: node ID format in skill.md corrected to `{parent_dir}_{filename_stem}_{entity}` — the old filename-only format caused ghost-duplicate nodes when AST and semantic extractors disagreed on the stem; top-level files use just the filename stem; existing graphs with ghost duplicates can be cleaned up with `graphify extract --full --force`
