...
def _get_whisper():
    try:
        ...
        raise ImportError(
            f"Video transcription requires faster-whisper. Run: {uv_tool_install_command('video')}"
        ) from exc
...
def _get_yt_dlp():
    try:
        ...
        raise ImportError(
            f"YouTube/URL download requires yt-dlp. Run: {uv_tool_install_command('video')}"
        ) from exc
