...
def _contains(result: dict, file: str, sym: str) -> bool:
    ...
    return any(e["target"] == tgt and e["relation"] == "contains" for e in result["edges"])


def test_generator_declaration_is_node_ts(tmp_path):
    f = _write(tmp_path / "src" / "g.ts", "export function* counter() { yield 1; yield 2; }\n")
...
def test_generator_declaration_is_node_js(tmp_path):
    f = _write(tmp_path / "src" / "g.js", "function* gen() { yield 42; }\n")
    r = extract([f], cache_root=tmp_path)
    assert _has_node(r, "src/g.js", "gen")


def test_generator_expression_is_node(tmp_path):
    f = _write(tmp_path / "src" / "h.ts", "export const stream = function* () { yield 'a'; };\n")
    r = extract([f], cache_root=tmp_path)
...
def test_generator_body_calls_are_attributed(tmp_path):
    ...
    assert any(
        e["source"] == src and e["target"] == tgt and e["relation"] == "calls" for e in r["edges"]
