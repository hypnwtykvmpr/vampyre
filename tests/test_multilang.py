...
def test_go_multisite_calls_are_keyed_by_source_location(tmp_path):
    ...
    # The multigraph contract here is set membership by call-site location; no
    # reader/export path depends on a same-pair parallel edge ordering contract.
    assert sorted(edge["source_location"] for edge in target_calls) == ["L6", "L7", "L8"]
