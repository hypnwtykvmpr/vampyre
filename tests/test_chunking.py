...
def test_pack_chunks_packs_small_files_together(tmp_path):
    ...
    assert len(chunks) == 1
    packed_paths = []
    for unit in chunks[0]:
        assert isinstance(unit, Path)
        packed_paths.append(unit)
    assert sorted(packed_paths) == sorted(files)
...
def test_pack_chunks_groups_by_directory(tmp_path):
    ...
    for unit in chunk:
        assert isinstance(unit, Path)
        packed_paths.append(unit)
    a_indices = [i for i, path in enumerate(packed_paths) if path.parent == dir_a]
    b_indices = [i for i, path in enumerate(packed_paths) if path.parent == dir_b]
