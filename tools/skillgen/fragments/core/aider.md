...
/graphify <path> --multigraph                         # preserve distinct parallel relationships
/graphify <path> --simple                             # explicitly select the historical simple graph
...
from graphify.persistence import capture_pending_signal
from pathlib import Path
signal = Path('graphify-out/needs_update')
generation = capture_pending_signal(signal)
Path('graphify-out/.graphify_signal_generation').write_bytes(generation or b'')
...
**Before starting:** replace `IS_DIRECTED` and `IS_MULTIGRAPH` everywhere. For a new graph, set them from the explicit `--directed`, `--multigraph`, or `--simple` mode (`--multigraph` implies directed). For an existing graph, inherit both from its validated `graphify_profile` unless the user explicitly selected a graph class. Never infer simple mode from missing profile state.
...
analysis_path = Path('.graphify_analysis.json')
